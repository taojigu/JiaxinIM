...
static int delayCentisecondsForImageAtIndex(CGImageSourceRef const source, size_t const i) {
    ...
    if (properties) {
        ...
        if (gifProperties) {
            NSNumber *number = fromCF CFDictionaryGetValue(gifProperties,
                                                           kCGImagePropertyGIFUnclampedDelayTime);
            if (number == NULL || [number doubleValue] == 0) {
                number = fromCF CFDictionaryGetValue(gifProperties, kCGImagePropertyGIFDelayTime);
            }
            if ([number doubleValue] > 0) {
                // Even though the GIF stores the delay as an integer number of centiseconds,
                // ImageIO “helpfully” converts that to seconds for us.
...
static void createImagesAndDelays(CGImageSourceRef source, size_t count,
                                  CGImageRef imagesOut[count], int delayCentisecondsOut[count]) {
...
static int vectorGCD(size_t const count, int const *const values) {
    ...
    for (size_t i = 1; i < count; ++i) {
        // Note that after I process the first few elements of the vector, `gcd` will probably be
        // smaller than any remaining element.  By passing the smaller value as the second argument
        // to `pairGCD`, I avoid making it swap the arguments.
...
static UIImage *animatedImageWithAnimatedGIFImageSource(CGImageSourceRef const source) {
    ...
    UIImage *const animation =
            [UIImage animatedImageWithImages:frames
                                    duration:(NSTimeInterval)totalDurationCentiseconds / 100.0];
